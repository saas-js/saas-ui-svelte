	...
	async function scrollSelectedIntoView() {
		...
		if (resultsRef && selectedIndex >= 0) {
			const selectedOption =
				resultsRef.querySelectorAll('[role="option"]')[selectedIndex];
						...
						<Box class="py-2" role="group" aria-label={group.label}>
							...
							{#each group.items.slice(0, 10) as item, i}
								...
								<Flex
									...
									align="center"
									gap={2}
									class="hover:bg-bg-subtle focus:bg-bg-subtle w-full cursor-pointer rounded px-2 py-2 text-left text-sm focus:outline-none {itemIndex ===
									selectedIndex
										? 'bg-bg-subtle'
										: ''}"
								>
