<script module lang="ts">
	...
	const animeMovies = createListCollection({
		items: [
			...
			{
				label: "Kiki's Delivery Service",
				value: "kikis_delivery_service",
			},
			...
			{
				label: "5 Centimeters per Second",
				value: "5_centimeters_per_second",
			},
...
{#snippet basicStory(args: any)}
	<div class="w-80">
		<Select.Root collection={frameworks} size="sm" {...args}>
...
<Story name="Option Group" template={optionGroupStory} />

<Story name="Disabled" template={disabledStory} />

<Story name="Invalid" template={invalidStory} />

<Story name="Avatar Select" template={avatarSelectStory} />

<Story name="Clear Trigger" template={clearTriggerStory} />
