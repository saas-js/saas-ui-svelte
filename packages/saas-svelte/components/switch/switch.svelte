	...
	const switchRecipe = tv({
		...
		variants: {
			...
			variant: {
				...
				raised: {
					control:
						"shadow-inner data-[state=checked]:bg-(--c-solid)/60",
	...
	<Switch.Control class={styles.control()}>
		...
		{#if trackLabel}
			<Box
				as="span"
				class={twMerge(
					"absolute grid size-(--switch-translate) shrink-0 content-center justify-center text-sm font-medium transition-transform duration-100 will-change-transform select-none",
				)}
