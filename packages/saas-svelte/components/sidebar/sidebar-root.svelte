<script lang="ts" module>
	...
	export const sidebar = tv({
		slots: {
			root: [
				"flex flex-col",
				"min-w-55 max-w-[100vw]",
				"h-full min-h-100",
				"border-r border-border-default",
				"bg-bg-panel",
				"transition-[width] duration-200 ease-[cubic-bezier(.34,1.24,0.64,1)]",
				"w-60",
				"antialiased",
			],
			...
			groupHeader: ["flex items-center", "px-2.5 py-1.5"],
			groupContent: ["flex flex-col", "pt-1"],
			navItem: ["text-[.8125rem] leading-[1.1375rem]", "relative py-px"],
			item: [
				"isolate",
				"flex items-center",
				"gap-2",
