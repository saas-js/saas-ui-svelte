	...
	export const timeline = tv({
		...
		variants: {
			size: {
				sm: {
					connector:
						"w-4 h-4 text-xs [&_svg]:size-3.5 [&:has(img)]:rounded-full [&:has(img)]:overflow-hidden",
				...
				md: {
					connector:
						"w-5 h-5 text-sm [&_svg]:size-3.5 [&:has(img)]:rounded-full [&:has(img)]:overflow-hidden",
				...
				lg: {
					connector:
						"w-6 h-6 text-sm [&_svg]:size-5 [&:has(img)]:rounded-full [&:has(img)]:overflow-hidden",
					content: "text-sm leading-5",
				...
				xl: {
					connector:
						"w-8 h-8 text-base [&_svg]:size-6 [&:has(img)]:rounded-full [&:has(img)]:overflow-hidden",
					content: "text-sm leading-5",
